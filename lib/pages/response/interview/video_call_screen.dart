...
class VideoCallScreen extends StatefulWidget {
  ...
  const VideoCallScreen(
      {Key key, this.employeeToken, this.employerToken, this.interviewID})
      : super(key: key);
...
class _VideoCallScreenState extends State<VideoCallScreen> {
  ...
  _getOfferFromLocal() async {
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      print(
          "------------------BROADCAST RECEIVED BY USER FINISHED------------------");
    });
  }
  ...
  _createOffer() async {
    ...
    var data = jsonEncode(
        {"notification_type": "getOffer", "interviewID": widget.interviewID});

    await GetAllInterviewsApi().sendInterviewSession(
        widget.interviewID, jsonEncode(session), userType);
  ...
  getUserMediaDevices() async {
    Map<String, dynamic> constraints = {"audio": false, "video": true};
