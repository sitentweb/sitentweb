...
class AllQuestions extends StatefulWidget {
  ...
  final fullQuiz;
  final quizRoomID;
  const AllQuestions({Key key, this.quizID, this.fullQuiz, this.quizRoomID}) : super(key: key);
...
class AllQuestionsState extends State<AllQuestions> {
  ...
  _createQuizDataForDatabase() async {
    ...
    await FetchQuizRoomApi().updateQuiz(widget.quizRoomID, jsonEncode(createData)).then((value) {
        print(value.status);
        var snackBar;
        if(value.status){
          snackBar = SnackBar(
            backgroundColor: Colors.green,
            content: Text("Questionnaire Submitted Successfully" , style: TextStyle(
              color: Colors.white,
  ...
  Future<bool> _onWillPop() async {
    return (await showDialog(
      ...
      builder: (context) => new AlertDialog(
        ...
        content: new Text('You will not be able to access this questionnaire again'),
        actions: <Widget>[
          TextButton(
          ...
          TextButton(
            onPressed: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Responses(initialIndex: 1,),)),
            child: new Text('Yes'),
          ),
        ],
      ),
                            ...
                            child: Column(
                              children: [
                                Container(
                                  ...
                                  child: Text("Question ${index + 1}" , style: GoogleFonts.lora(
                                    color: Colors.white
                                  ),
                                  ),
                                ),
                                        ...
                                        return InkWell(
                                          ...
                                          child: Container(
                                            ...
                                            decoration: BoxDecoration(
                                              ...
                                              color: _selectOption != null && _selectOption == opIndex ?  Colors.green[200] : Colors.grey[300]
                                            ),
                                            padding: EdgeInsets.all(15),
                                            child: Text("${q.options[opIndex]}"),
                                          ),
                                        );
                                      },
                                  ),
                                      ...
                                      !isLoading ? MaterialButton(
                                        ...
                                        onPressed: () {
                                          ...
                                          if(_questions.length == index+1){
                                            _onSelectFinalOption(_selectOption);
                                        ...
                                        child: _questions.length == index+1 ? Text("Submit" , style: TextStyle(
                                          color: Colors.white
